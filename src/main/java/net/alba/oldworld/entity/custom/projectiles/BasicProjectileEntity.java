...
public class BasicProjectileEntity extends ProjectileEntity{
    private float damage;
    private StatusEffectInstance status;
    private boolean giveFire;

    public BasicProjectileEntity(EntityType<? extends ProjectileEntity> entityType, World world) {
    ...
    public BasicProjectileEntity(World world, LivingEntity owner, double velocityX, double velocityY, double velocityZ, float damage, StatusEffectInstance effect, boolean fire) {
        super(ModEntities.PROJECTILE_THROW, owner, velocityX, velocityY, velocityZ, world);
        this.damage = damage;
        this.status = effect;
        this.giveFire = fire;
    }
